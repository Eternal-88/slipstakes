...
(function (G) {
  ...
  function trackHead(st, label) {
    ...
    if (!tr) return '';
    // (v5.1: this race is an endurance race — always on Endurance Park,
  ...
  const Entry = {
    ...
    render() {
      ...
      const key = me.carId + JSON.stringify(me.garage.installed) + JSON.stringify(me.garage.wear) + JSON.stringify(me.garage.tune);
      if (this._key !== key) {
        this._key = key;
        this._spec = Parts.computeSpec(me.carId, me.garage.installed, me.garage.wear, me.garage.tune);
        this._cost = Parts.computeStats(this._spec).cost;
      }
      const spec = this._spec, cost = this._cost;
