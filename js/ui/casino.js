...
(function (G) {
  ...
  const Casino = {
    ...
    render() {
      ...
      UI.patch(
        this.el.head,
        `${UI.interTabs()}<div class="cz-tabs"><button class="${this.tab === 'bj' ? 'on' : ''}" data-act="ctab" data-t="bj">🃏 Blackjack</button><button class="${this.tab === 'rl' ? 'on' : ''}" data-act="ctab" data-t="rl">🎡 Roulette</button></div>
         <div class="cz-money"><span>CASH</span><b>${U.fmtMoney(me.money)}</b><em>floor ${U.fmtMoney(G.Econ.FLOOR)}</em></div>
         <span class="muted small">${G.Game.readyLine()}</span><button class="btn ${me.ready ? 'green' : 'primary'}" data-act="ready">${me.ready ? '✓ Ready' : 'Ready'}</button>`
      );
      ...
      if (!cz) {
        UI.patch(this.b.ctl, '<p class="muted">Tables opening…</p>');
        return;
      }
