...
(function (G) {
  ...
  const Game = {
    ...
    async hostNew(name) {
      ...
      s.addPlayer({ id: HOST_PID, name: name || 'Host', color: G.CarModel.PALETTE[0], carId: 'vandal' });
      s.syncBots();
    ...
    tick(dt) {
      ...
      if (this.role === 'host') {
        ...
        if (this.hostRace) {
          ...
          if (drivingHere && G.Input.hit('KeyR')) inp.rs = 1;
    ...
    render(dt) {
      ...
      if (st.phase === 'race' && (this.hostRace || this.clientRace)) {
        ...
        if (G.Input.hit('KeyC')) {
          app.world.cam.mode = app.world.cam.mode === 'follow' ? 'fixed' : 'follow';
          U.store.set('ss.cam', app.world.cam.mode);
        }
        ...
        if (st.phase === 'intermission' && G.UI.curName === 'garage') G.Preview.update(dt);
        else {
