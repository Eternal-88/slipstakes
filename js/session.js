  ...
  class HostSession extends U.Emitter {
    ...
    syncBots() {
      ...
      while (this.bots().length < want) {
        ...
        this.addPlayer({ id, name: G.BOT_NAMES[n % G.BOT_NAMES.length] + ' ⚙', isBot: true, color, carId: G.Parts.CAR_ORDER[(n + 1) % 4], botSkill: 0.88 + 0.025 * (n % 4) });
        if (++k > 8) break;
      }
      this.touch();
    }
