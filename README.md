...
- v5.5.3 **Online players and direct messages** (`js/ui/online.js`, the 👥 corner button). Every open game keeps a retained "online card" on the relay's MQTT brokers (`slipstakes/online/v1/p/<tab id>`, with a Last Will that wipes it when the tab goes, refreshed every 25 s): name, status, and the code of its room only if that room is public. DMs go to `.../dm/<tab id>`, AES-GCM with a key from ECDH of the sender's private and the recipient's public key (`G.Relay.DM`), so only the recipient can read one and it can only have come from the card that sent it. Settings `showOnline` and `allowDM`, a per-session Block, and nothing is stored.
